//------------------------------------------------------------------------------
// LAGraph_TriangleCount_basic: count the number of triangles in a graph
//------------------------------------------------------------------------------
...
)
{
    ...
    GrB_Index *col_ids;
    GrB_Index  nv   = nvals;
    row_ids = (GrB_Index*) LAGraph_Malloc(nvals, sizeof(GrB_Index), &sz);
    col_ids = (GrB_Index*) LAGraph_Malloc(nvals, sizeof(GrB_Index), &sz);

    if (type == GrB_BOOL)
    {
        bool *values = LAGraph_Malloc(nvals, sizeof(bool), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_INT8)
    {
        int8_t *values = LAGraph_Malloc(nvals, sizeof(int8_t), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_INT16)
    {
        int16_t *values = LAGraph_Malloc(nvals, sizeof(int16_t), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_INT32)
    {
        int32_t *values = LAGraph_Malloc(nvals, sizeof(int32_t), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_INT64)
    {
        int64_t *values = LAGraph_Malloc(nvals, sizeof(int64_t), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_UINT8)
    {
        uint8_t *values = LAGraph_Malloc(nvals, sizeof(uint8_t), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_UINT16)
    {
        uint16_t *values = LAGraph_Malloc(nvals, sizeof(uint16_t), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_UINT32)
    {
        uint32_t *values = LAGraph_Malloc(nvals, sizeof(uint32_t), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_UINT64)
    {
        uint64_t *values = LAGraph_Malloc(nvals, sizeof(uint64_t), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_FP32)
    {
        float *values = LAGraph_Malloc(nvals, sizeof(float), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else if (type == GrB_FP64)
    {
        double *values = LAGraph_Malloc(nvals, sizeof(double), &vsz);
        GrB_TRY( GrB_Matrix_extractTuples(row_ids, col_ids, values, &nv, A) );
        LAGraph_Free((void**)&values, vsz);
    }
    else
    {
        // unknown type
        LAGraph_Free((void**)&row_ids, sz);
        LAGraph_Free((void**)&col_ids, sz);
        return -2;
    }
    ...
    if (L != NULL)
    {
        ...
        GrB_Index *lcol_ids;
        bool      *lvals;
        lrow_ids = (GrB_Index*) LAGraph_Malloc(l_nvals, sizeof(GrB_Index), &lsz);
        lcol_ids = (GrB_Index*) LAGraph_Malloc(l_nvals, sizeof(GrB_Index), &lsz);
        lvals    = (bool*)      LAGraph_Malloc(l_nvals, sizeof(bool), &lvsz);
        GrB_Index idx = 0;
        for (GrB_Index ix = 0; ix < nvals; ++ix)
        {
            if (row_ids[ix] > col_ids[ix])
        ...
        GrB_TRY( GrB_Matrix_build(*L, lrow_ids, lcol_ids, lvals, idx,
                                  GrB_SECOND_BOOL) );
        LAGraph_Free((void**)&lrow_ids, lsz);
        LAGraph_Free((void**)&lcol_ids, lsz);
        LAGraph_Free((void**)&lvals, lvsz);
#else
    ...
    if (U != NULL)
    {
        ...
        GrB_Index *ucol_ids;
        bool      *uvals;
        urow_ids = (GrB_Index*) LAGraph_Malloc(u_nvals, sizeof(GrB_Index), &usz);
        ucol_ids = (GrB_Index*) LAGraph_Malloc(u_nvals, sizeof(GrB_Index), &usz);
        uvals    = (bool*)      LAGraph_Malloc(u_nvals, sizeof(bool), &uvsz);
        GrB_Index idx = 0;
        for (GrB_Index ix = 0; ix < nvals; ++ix)
        {
            if (row_ids[ix] < col_ids[ix])
        ...
        GrB_TRY( GrB_Matrix_build(*U, urow_ids, ucol_ids, uvals, idx,
                                  GrB_SECOND_BOOL) );
        LAGraph_Free((void**)&urow_ids, usz);
        LAGraph_Free((void**)&ucol_ids, usz);
        LAGraph_Free((void**)&uvals, uvsz);
#else
